...
export class WebController {
  ...
  @Get("/")
  home() {
    this.logger.info("Rendering cached home page");
    return view("home", {
  ...
  article(@Param("slug") slug: string) {
    this.logger.info(`Rendering cached article page for ${slug}`);
    return view("article", {
      title: `Article ${slug}`,
      slug,
  ...
  articleApi(@Param("slug") slug: string) {
    this.logger.info(`Serving cached article JSON for ${slug}`);
    return {
      slug,
      title: `Article ${slug}`,
  ...
  @Get("/preview")
  preview() {
    this.logger.info("Rendering uncached preview page");
    return view("preview", {
      title: "Draft Preview",
